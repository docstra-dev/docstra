...
class DocstraSession:
    ...
    def __init__(self, session_id: Optional[str] = None, config: Optional[DocstraConfig] = None):
    ...
    @classmethod
    def from_database(cls, session_id: str, created_at: str, config_json: str, 
                     messages: List[Dict[str, str]]) -> 'DocstraSession':
        """Create a session from database records.
        
        Args:
            session_id: The session ID from the database
            created_at: ISO format datetime string
            config_json: JSON string containing session configuration
            messages: List of message dictionaries
            
        Returns:
            A restored DocstraSession instance
        """
        # Create session
